...
读者不需要 ChatGPT、Codex 或微信登录。网站由 GitHub Actions 生成并发布到 GitHub Pages；个人电脑关机后也会继续运行。微信公众号暂不依赖群发 API，而是提供一份可直接复制到公众号后台的富文本排版稿，由管理员预览后手动发布。
...
```text
北京时间 09:07 主任务 / 09:22 失败补跑
                 ↓
OpenAI 联网扫描 → 结构与来源校验 → 机器核验首次发布时间
                 ↓
       生成静态 JSON 与网页 → GitHub Pages
                 ↓
  /wechat.html 富文本排版 → 人工复制、预览并发布公众号
```

- 信息窗口固定为北京时间 `[前一日 09:00, 当日 09:00)`；09:00 整发布的内容归入下一期。
- 09:07 是主任务，09:22 是兜底任务。兜底发现当日期刊已生成时不会再次调用模型，只会重新检查并发布网页。
- 新一期只有在十条信息全部通过结构、来源和时间校验后才会替换首页。任何一步失败，线上继续保留上一期。
- GitHub 的定时任务可能出现几分钟延迟，因此这里承诺的是“每天 09:00 截止、09:00 后尽快更新”，不是 09:00:00 准点刷新。
...
公共仓库可使用 GitHub-hosted runner 与 GitHub Pages。Cloudflare、D1、Wrangler 和微信 AppSecret 都不是这套方案的前置条件。OpenAI API 按实际调用计费，仍需由仓库维护者自行准备 API Key 与账户额度。
...
2. 再手动运行 `Generate daily AI brief`，确认联网生成、校验、提交和页面发布全部成功。
3. 之后两个定时任务会自动接管，无需保持电脑或 Codex 在线。
...
本地不填写 API Key 也可以浏览仓库自带的最新一期；只有手动执行生成命令时才需要密钥。
...
- `.github/workflows/daily-brief.yml`：每日生成与发布。
...
- OpenAI API 不是免费的。模型、搜索上下文和重试次数都会影响成本，建议在 OpenAI 后台设置用量上限与告警。
